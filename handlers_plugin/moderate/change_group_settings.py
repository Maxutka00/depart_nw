...
async def change_settings(app: Client, message: Message):
    ...
    if len(message.command) < 2:
        await message.reply("Вы не указали аргументы\n/help для показа всех команд")
        return
    if message.command[1] not in list(settings_commands):
        await message.reply("Такой настройки нет\n/help для показа всех команд")
        return
    ...
    if command == 'admins' and len(message.text.split(maxsplit=2)) >= 3:
        if args[1] == 'add':
            ...
            await message.reply("Отправил вам в лс")
            return
        else:
        ...
        if args[1] == 'start' or args[1] == 'stop':
            ...
            for i in time:
                if not i.isdigit():
                    await message.reply("Время в неправильном формате")
                    return
        ...
        if args[1] == 'add':
            ...
            await message.reply("Ссылки успешно добавлены")
        elif args[1] == 'del':
            links = args[2].split(';')
            for link in links:
                db.del_link_from_whitelist(message.chat.id, link)
            await message.reply("Ссылки успешно удалены")
        elif args[1] == 'enable':
        ...
        if args[1] == 'add':
            ...
            await message.reply("Слова успешно добавлены")
        elif args[1] == 'del':
            words = args[2].split(';')
            for word in words:
                db.del_word_from_blacklist(message.chat.id, word)
            await message.reply("Слова успешно удалены")
        elif args[1] == 'clean':
            pass
        elif args[1] == 'get':
